...
- Знак-модуль: знак живёт в отдельном поле `Sign`, модуль — в поле `limbs`
  (цифры числа по основанию 2³², младшая первая — каждый элемент хранит одну
  32-битную «цифру»):
...
- `+`/`-`/унарный `-`/`*` десугарятся в `@plus`/`@minus`/`@neg`/`@times`
  соответственно; у `@abs` операторной формы нет. Все бесотказные и
  точные.
