...
| Ф.4 Строки и связка семьи | **КРАСНЫЙ — СТОП** (дефект компилятора: вложенные `Err(Variant)`-паттерны эмитят теги чужого enum'а) | — |
...
- `nova test src`: **PASS 8 / FAIL 0 / SKIP 3** на Ф.0–Ф.3; Ф.4 добавлена, но гейт
  падает на матчинге вариантов `ParseBigRatError`.
- СТОП-репорт: `REPORT-p240.md` (история; Ф.0-блокер закрыт, Ф.4-блокер открыт —
  см. разделы в конце файла).
- Дальше: Ф.5/Ф.6 НЕ начинать до решения владельца по Ф.4-дефекту кодогенератора.
